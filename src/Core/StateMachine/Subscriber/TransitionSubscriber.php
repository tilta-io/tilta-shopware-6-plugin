...
class TransitionSubscriber implements EventSubscriberInterface
{
    ...
    public function onTransition(StateMachineTransitionEvent $event): void
    {
        ...
        /** @var TiltaOrderDataEntity|null $billieData */
        $billieData = $order->getExtension(OrderDataEntityExtension::EXTENSION_NAME);
        if (!$billieData instanceof TiltaOrderDataEntity) {
        ...
        switch ($event->getToPlace()->getTechnicalName()) {
            case $this->configService->getStateForShip():
                $this->operationService->createInvoice($order);
                break;
            case $this->configService->getStateCancel():
                $this->operationService->cancelOrder($order);
                break;
            case $this->configService->getStateReturn():
                $this->operationService->refundInvoice($order);
                break;
        }
    }
